...
namespace RainMeadow.UI.Components
{
            ...
            SelectOneButton.SelectOneButtonOwner
    {
        ...
        public bool IsTeamsAddedInCorrectly =>
            !teamBattleMode.teamNames.All(x =>
                teamBattleMode.teamColors.ContainsKey(x.Key)
                && teamBattleMode.teamIcons.ContainsKey(x.Key)
            )
            || !(
                teamBattleMode.teamNames.Count == teamBattleMode.teamIcons.Count
                && teamBattleMode.teamNames.Count == teamBattleMode.teamColors.Count
            );
        public int CurrentOffset
        {
            ...
            set =>
                currentOffset = Mathf.Clamp(
                    value,
                    0,
                    (teamBattleMode.teamNames.Count > 0)
                        ? ((teamBattleMode.teamNames.Count - 1) / 4)
                        : 0
                );
        }
            ...
            : base(menu, owner, pos, size)
        {
            ...
            teamLerpTextBox = new(
                new Configurable<float>(teamBattleMode.lerp),
            ...
            )
            {
                ...
                description = menu.Translate(
                    "How much do you want team color to show on players? From 0 to 1"
                ),
            };
            ...
            friendlyFireCheckbox = new(
                new Configurable<bool>(arena.friendlyFire),
                new(teamLerpTextBox.pos.x * 0.5f - 30, 20)
            );
            ...
            teamColorLerpLabel = new(
                menu,
                this,
                menu.Translate("Team Lerp Color:"),
                new(teamLerpTextBox.pos.x, teamLerpTextBox.pos.y + teamLerpTextBox.size.y + 10),
                new(teamLerpTextBox.size.x, 0),
                false
            );
            friendlyFireLabel = new(
                menu,
                this,
                menu.Translate("Friendly Fire:"),
                new(
                    friendlyFireCheckbox.pos.x,
                    friendlyFireCheckbox.pos.y + friendlyFireCheckbox.size.y + 10
                ),
        ...
        public void PopulatePage(int offset)
        {
            ...
            if (IsTeamsAddedInCorrectly)
                throw new NotImplementedException(
                    "teamNames, teamMappings and teamColors do not have all their keys"
                );
            ...
            for (int i = 0; i < teamButtons.Length; i++)
            {
                ...
                Vector2 btnpos =
                    i == 0 ? new(posXMultipler - 50, size.y - 125)
                    : i % 2 == 0 ? new(teamButtons[0].pos.x, teamButtons[0].pos.y - 140)
                    : new(posXMultipler * 3 - 50, teamButtons[i - 1].pos.y);
                teamButtons[i] = new(
                    menu,
                    this,
                    btnpos,
                ...
                Vector2 namePickerPos =
                    i == 0 ? new(posXMultipler - 75, dividerY - 40)
                    : i % 2 == 0 ? new(teamNameBoxes[0].pos.x, teamNameBoxes[0].pos.y - 40)
                    : new(posXMultipler * 3 - 95, teamNameBoxes[i - 1].pos.y);
                OpTextBox textBox = teamNameBoxes[i] = new(
                    new Configurable<string>(name),
                    namePickerPos,
                    150
                );
                textBox.allowSpace = true;
                textBox.accept = OpTextBox.Accept.StringASCII;
                textBox.OnValueUpdate += (config, value, oldValue) =>
                    NameTextBox_OnValueUpdated(mapping.Key, value);
                new PatchedUIelementWrapper(tabWrapper, textBox);
                OpTinyColorPicker tinyPicker = teamColorPickers[i] = new(
                    menu,
                    new(namePickerPos.x + 10 + textBox.size.x, namePickerPos.y),
                    teamColor,
                    tabWrapper
                );
        ...
        public override void Update()
        {
            ...
            for (int i = 0; i < teamButtons.Length; i++)
            {
                ...
                teamBtn.teamName = NetworkDomain.currentInstance.FilterTeamName(
                    teamBattleMode.teamNames[actualTeamIndex]
                );
                teamBtn.buttonBehav.greyedOut = teamColorPickers.Any(x => x.currentlyPicking);
                teamBtn.teamCount.text = OnlineManager
                    .lobby.clientSettings.Where(x =>
                        OnlineManager.players.Contains(x.Key)
                        && x.Value.TryGetData<ArenaTeamClientSettings>(out var team)
                        && team.team == actualTeamIndex
                    )
                    .Count()
                    .ToString();
                ...
                if (!teamNameBox.held)
                    teamNameBox.value = name;

                if (!teamColorPicker.held)
                    teamColorPicker.valuecolor = color;

                bool greyOutConfig =
                    OwnerSettingsDisabled
                    || teamColorPickers.Any(x => x.currentlyPicking && x != teamColorPickers[i]);
