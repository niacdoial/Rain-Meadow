...
public class ArenaMainLobbyPage : PositionedMenuObject, IDynamicBindHandler
{
        ...
        : base(menu, owner, pos)
    {
        ...
        readyButton.description = Utils.Translate(
            scugslotsHint == 20
                ...
                : "Ready up to join the host when the match begins"
        );
        ...
        activeGameModeLabel = new MenuLabel(
            menu,
            this,
            "",
            new Vector2(chatMenuBox.pos.x + chatMenuBox.size.x / 2, chatRectPosSizeY - 15),
            Vector2.zero,
            false
        );
        readyPlayerCounterLabel = new MenuLabel(
            menu,
            this,
            "",
            new Vector2(chatMenuBox.pos.x + chatMenuBox.size.x / 4, chatRectPosSizeY - 35),
            Vector2.zero,
            false
        );
        playlistProgressLabel = new MenuLabel(
            menu,
            this,
            "",
            new Vector2(chatMenuBox.pos.x + chatMenuBox.size.x / 4 * 3 - 20, chatRectPosSizeY - 35),
            Vector2.zero,
            false
        );
        ...
        playListTab.AddObjects(
            levelSelector = new ArenaLevelSelector(menu, playListTab, new Vector2(65f, 7.5f))
        );
        tabContainer.AddTab(playListTab, menu.Translate("Arena Playlist"));

        matchSettingsTab = new(menu, tabContainer);
        arenaSettingsInterface = new OnlineArenaSettingsInferface(
            menu,
            matchSettingsTab,
            new Vector2(120f, 0f),
    ...
    {
        playerDisplayer = new PlayerDisplayer(
            menu,
            this,
            new Vector2(960f, 130f),
            [.. OnlineManager.players.OrderByDescending(x => x.isMe)],
            GetPlayerButton,
            4,
    ...
    {
        RainMeadow.DebugMe();
        playerDisplayer?.UpdatePlayerList([
            .. OnlineManager.players.OrderByDescending(x => x.isMe),
        ]);
    }
    ...
    )
    {
        if (isLargeDisplay)
        {
            ArenaPlayerBox playerBox = new(
                menu,
                playerDisplay,
                player,
                OnlineManager.lobby?.isOwner == true,
                pos
            ); //buttons init prevents kick button if isMe
            return playerBox;
        }
        ArenaPlayerSmallBox playerSmallBox = new(
            menu,
            playerDisplay,
            player,
            OnlineManager.lobby?.isOwner == true,
    ...
    public void OpenColorConfig(SlugcatStats.Name? slugcat)
    {
        ...
        dialog = new ColorMultipleSlugcatsDialog(
            menu.manager,
            () =>
            {
                menu.PlaySound(SoundID.MENU_Button_Standard_Button_Pressed);
    ...
    public void OnTabButtonsCreated()
    {
        ...
        menu.TrySequentialMutualBind(
            [
                arenaInfoButton,
                tabContainer.tabButtonContainer.topArrowButton,
                playerDisplayer.scrollUpButton,
            ],
            true,
            loopLastIndex: true
        );
    ...
    public void BindPlaylistTabSelectables()
    {
        ...
        menu.TrySequentialMutualBind(
            [
                arenaInfoButton,
                tabContainer.tabButtonContainer.topArrowButton,
                levelSelector.allLevelsPlaylist.scrollUpButton,
                levelSelector.selectedLevelsPlaylist.scrollUpButton,
                playerDisplayer.scrollUpButton,
            ],
            true,
            loopLastIndex: true
        ...
        menu.MutualHorizontalButtonBind(
            levelSelector.selectedLevelsPlaylist.scrollDownButton,
            playerDisplayer.scrollDownButton
        );
    ...
    public void BindSelectSettingsPage(bool isHidden)
    {
        ...
        menu.MutualHorizontalButtonBind(
            (abilitiesTabBtn ?? tabButtons[0]).wrapper,
            selectSettings.scroller.scrollSlider
        );
    }
    ...
    public void UpdatePlayerButtons(ButtonScroller.IPartOfButtonScroller button)
    {
        ...
        {
            ArenaClientSettings? clientSettings = ArenaHelpers.GetArenaClientSettings(
                playerBox.profileIdentifier
            );
            ...
            )
            {
                ...
                playerBox.slugcatButton.LoadNewSlugcat(
                    clientSettings?.playingAs,
                    clientSettings != null && clientSettings.slugcatColor != Color.black,
                    false
                );
            ...
            if (clientSettings?.selectingSlugcat == true)
                playerBox.ToggleTextOverlay("Selecting<LINE>Slugcat", true);
            if (
                Arena.arenaSittingOnlineOrder.Contains(playerBox.profileIdentifier.inLobbyId)
                && Arena.isInGame
            )
                playerBox.ToggleTextOverlay("In Game!", true);

            Color color =
                playerBox.slugcatButton.isColored && clientSettings != null
                    ? clientSettings.slugcatColor
                    : Color.white;
            if (Arena.externalArenaGameMode != null)
                color = Arena.externalArenaGameMode.GetPortraitColor(
                    Arena,
                    playerBox.profileIdentifier,
                    color
                );
        ...
        if (button is ArenaPlayerSmallBox smallPlayerBox)
            smallPlayerBox.slugcatButton.slug = ArenaHelpers
                .GetArenaClientSettings(smallPlayerBox.profileIdentifier)
                ?.playingAs;
    }

    public void UpdateMatchButtons()
    {
        readyButton.buttonBehav.greyedOut =
            (!Arena.allowJoiningMidRound && Arena.arenaClientSettings.ready)
            || (OnlineManager.lobby.isOwner && Arena.initiateLobbyCountdown);
        readyButton.menuLabel.text = menu.Translate(
            Arena.arenaClientSettings.ready
                ? !Arena.allowJoiningMidRound
                    ? "WAITING"
                    : "UNREADY"
                : "READY?"
        );
        ...
        startButton.buttonBehav.greyedOut =
            !Arena.arenaClientSettings.ready
            || levelSelector.SelectedPlayList.Count == 0
            || Arena.initiateLobbyCountdown;
        startButton.menuLabel.text = Arena.initiateLobbyCountdown
            ? menu.Translate(Arena.lobbyCountDown.ToString())
    ...
    public override void Singal(MenuObject sender, string message)
    {
        ...
        {
            SlugcatStats.Name? slug =
                sender?.owner is ArenaPlayerBox playerBox ? playerBox.slugcatButton.slugcat
                : sender?.owner is ArenaPlayerSmallBox smallPlayerBox
                    ? smallPlayerBox.slugcatButton.slug
                : null;
    ...
    public override void Update()
    {
        ...
        {
            if (
                menu.selectedObject.Selected
                && (
                    (menu.selectedObject is SimpleButton btn && btn.signalText == "CHANGE_SLUGCAT")
                    || (
                        menu.selectedObject is SlugcatColorableButton col
                        && col.signalText == "CHANGE_SLUGCAT"
                    )
                )
            )
        ...
        activeGameModeLabel.text = LabelTest.TrimText(
            $"{menu.Translate("Current Mode:")} {menu.Translate(Arena.currentGameMode)}",
            chatMenuBox.size.x - 10,
            true
        );
        readyPlayerCounterLabel.text =
            $"{menu.Translate("Ready:")} {ArenaHelpers.GetReadiedPlayerCount(OnlineManager.players)}/{OnlineManager.players.Count}";
        int amtOfRooms = ArenaMenu?.GetGameTypeSetup?.playList != null
                ? ArenaMenu.GetGameTypeSetup.playList.Count
                : 0,
            amtOfRoomsRepeat = arenaSettingsInterface?.roomRepeatArray != null
                ? arenaSettingsInterface.roomRepeatArray.CheckedButton + 1
                : 0;
        playlistProgressLabel.text =
            $"{menu.Translate("Playlist Progress:")} {Arena.currentLevel}/{(Arena.isInGame ? Arena.totalLevelCount : (amtOfRooms * amtOfRoomsRepeat))}";

        if (OnlineManager.lobby.isOwner)
        {
            if (menu.manager.upcomingProcess == null)
